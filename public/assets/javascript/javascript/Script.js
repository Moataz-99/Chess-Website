	const chessboardParent = document.getElementById("chessboard"); //point to the playboard div
	const getButton = document.getElementById("G");
	...
	class Chess {
		...
		async assignPlayers() {
			...
			return new Promise((resolve) => {
				const player1 = new Player({ username: "Me", id: 1, role: "white" }); // player 1
				const player2 = new Player({ username: "Arm", id: 2, role: "black" }); // player 2 
		...
		moved(...param) {
			...
			this.isMate(); 
			this.updatePlayers();
			this.insertToMatchHistory(...param);
			// this.previewMatchHistory(...param)
		}
		...
		insertToMatchHistory(from, to) {
			...
			if(this.info.turn == this.data.players[1]){
				console.log('white')
				fetch('/send', {  
					method: 'POST',
	...
	class Player {
		constructor(player) {
			this.info = {
				// isRobot:false ,// is the robot turn
	...
	Game.init(function () { // intializing the game
		this.start();
		this.insertToMatchHistory("./assets/javascript/json/matchhistory.json");
	...
	getButton.addEventListener('click', event => {
		...
		let Mv = {piece:"wp1",from:"b2",to:"b3"}
			...
			console.log('hi')
				...
				const moveTo = async function (player, piece, square) {
					...
					let player = Game.data.players[1]; // the player
					let piece = board.filterPiece(Mv.piece); // convert it to piece class
					let square = board.filterSquare(Mv.to); // find the square
					let move =   moveTo(player, piece, square) // move the piece
